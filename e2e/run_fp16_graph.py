"""End-to-end bs=1 decoding throughput, FP16 Llama-2-7B -- CUDA GRAPH (torch.compile max-autotune).

Wraps `decode_one` in `torch.compile(mode='max-autotune', fullgraph=True)`,
which triggers Inductor's reduce-overhead path: kernel autotuning +
CUDA-graph capture of the compiled decode step. This is the mode the QTIP
paper's own `interactive_gen.py` uses and what any real inference stack
(vLLM, SGLang, TensorRT-LLM, ...) would use for bs=1 decode.
...
def main():
    ...
    # Eager warmup to shake out kernel JIT before compile.
